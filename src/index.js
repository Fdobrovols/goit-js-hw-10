...
function renderMarkup(data) 
{
  ...
  if (data.length >= 2 && data.length <= 10) 
  {
      ...
      .map(({ name, flags }) => 
      {
        return `<li class="country__item"><img src="${flags.svg}" alt="${flags.alt}" class="country__img" /><h2 class="country__heading">
        ${name.official}</h2></li>`;
      }).join('');
      insertHtml(markup);
    return;
  ...
  const allLanguages = Object.entries(languages)
    ...
    <img src="${flags.svg}" alt="${flags.alt}" class="country-info__img" />
        ...
        <li class="country-info__item"><span class="country-info__name">Language${Object.keys(languages).length === 1 ? '' : 's'}: 
